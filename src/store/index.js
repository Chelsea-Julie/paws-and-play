...
export default createStore({
  ...
  actions: {
    ...
    async fetchProducts(context) {
      try {
        const { results } = await (await axios.get(`${apiURL}services`)).data
        if (results) {
    ...
    async recentProducts(context) {
      try {
        const { results, msg } = await (await axios.get(`${apiURL}services/recent`)).data
        if (results) {
    ...
    async fetchProduct(context, id) {
      try {
        const { result, msg } = await (await axios.get(`${apiURL}services/${id}`)).data
        if (result) {
    ...
    async addAProduct(context, payload) {
      try {
        const { msg } = await (await axios.post(`${apiURL}services/add`, payload)).data
        if (msg) {
    ...
    async updateProduct(context, payload) {
      try {
        const { msg } = await (await axios.patch(`${apiURL}services/${payload.productID}`, payload)).data
        if (msg) {
          context.dispatch('fetchProducts')
          toast.success(`${msg}`, {
            autoClose: 2000,
    ...
    async deleteProduct(context, id) {
      try {
        const { msg } = await (await axios.delete(`${apiURL}services/${id}`)).data
        if (msg) {
          context.dispatch('fetchProducts')
          toast.success(`${msg}`, {
            autoClose: 2000,
